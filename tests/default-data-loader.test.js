...
(async () => {
  ...
  if (context.DQ.DEFAULT_GAME_DATA.schemaVersion !== 13 || context.DQ.DEFAULT_GAME_DATA.ai.turnOrder.maxMultiplier !== 1.25 || context.DQ.DEFAULT_GAME_DATA.ai.targetSelection.enemyFrontWeight !== 5 || context.DQ.DEFAULT_GAME_DATA.jobs.length !== 3 || context.DQ.DEFAULT_GAME_DATA.enemies.length < 15 || !context.DQ.DEFAULT_GAME_DATA.jobs[0].levelStats["24"] || context.DQ.DEFAULT_GAME_DATA.actions.some(action => !Array.isArray(action.effects) || !action.effects.length) || !context.DQ.DEFAULT_GAME_DATA.actions.some(action => action.id === "zaoriku")) {
