...
(async () => {
  ...
  if (context.DQ.DEFAULT_GAME_DATA.schemaVersion !== 14 || context.DQ.DEFAULT_GAME_DATA.ai.turnOrder.maxMultiplier !== 1.25 || context.DQ.DEFAULT_GAME_DATA.ai.targetSelection.enemyFrontWeight !== 5 || context.DQ.DEFAULT_GAME_DATA.jobs.length !== 5 || context.DQ.DEFAULT_GAME_DATA.enemies.length < 50 || !context.DQ.DEFAULT_GAME_DATA.jobs.every(job => job.levelStats["50"]) || context.DQ.DEFAULT_GAME_DATA.actions.some(action => !Array.isArray(action.effects) || !action.effects.length) || !context.DQ.DEFAULT_GAME_DATA.actions.some(action => action.id === "gigadein") || !context.DQ.DEFAULT_GAME_DATA.jobs.some(job => job.id === "hero") || !context.DQ.DEFAULT_GAME_DATA.jobs.some(job => job.id === "sage")) {
